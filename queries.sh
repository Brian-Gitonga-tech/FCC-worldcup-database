...
echo "$($PSQL "SELECT year,name FROM teams INNER JOIN games ON teams.team_id=games.winner_id WHERE year=2014 AND round='Final' OR year=2018 AND round='Final' ORDER BY(year)")"

echo -e "\nList of teams that start with 'Co':"
echo "$($PSQL "SELECT name FROM teams INNER JOIN games ON teams.team_id=games.winner_id WHERE name ILIKE 'Co%'")"
